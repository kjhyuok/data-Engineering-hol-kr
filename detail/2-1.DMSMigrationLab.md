...
6. (Optional) 변경분에 대한 지속적인 복제를 위해 CDC endpoint 생성
7. (Optional) 변경분에 대한 지속적인 복제를 위해 Task 생성
- 다음 단계
...
부가적으로, you can add ongoing replication of data changes on the 
source: (Only one of the DMS replication instances will enable this 
feature.)

1. Create target endpoint for CDC files to place these files in a separate location than the initial load files
2. Create a task to perform the ongoing replication of data changes
...
1. On the [DMS console](https://console.aws.amazon.com/dms/v2/home#createSubnetGroup)

, select Subnet Groups and Create subnet group.
...
### 7. (Optional) 변경분에 대한 지속적인 복제를 위해 CDC endpoint 생성
...
### 8. (Optional) 변경분에 대한 지속적인 복제를 위해 Task 생성
